...
<div class="footer">
  ...
  <p>Frameworks covered: NIST AI RMF 1.0, FDA SaMD, ONC HTI-1, HIPAA, State AI Laws</p>
    ...
    await ctx.auth.auditLog(ctx.user.tenant_id, ctx.user.user_id, 'read', 'compliance_report', null,
      { report_type: 'audit_pack', framework: framework || 'all', control_count: total, compliance_pct: compliancePct },
...
<div class="footer">
  <p>Generated by ForgeAI Govern&trade; Healthcare AI Governance Platform. This profile is suitable for FDA submissions, Joint Commission reviews, and internal governance audits.</p>
    ...
    const phiRelated = asset.phi_access ? true : false;
    await ctx.auth.auditLog(ctx.user.tenant_id, ctx.user.user_id, 'read', 'asset_profile', id,
      { asset_name: asset.name, phi_access: asset.phi_access, risk_count: risks.results.length, incident_count: incidents.results.length },
      { dataClassification: phiRelated ? 'phi' : 'sensitive' });
