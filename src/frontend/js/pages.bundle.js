(() => {
  ...
  var dashboard = {
    async dashboard() {
      ...
      const risk2 = d.ai_portfolio.risk_distribution;
      ...
      <div class="stats-row">
        ...
        <div class="stat-card critical"><div class="stat-value">${(risk2.critical || 0) + (risk2.high || 0)}</div><div class="stat-label">High/Critical Risk Systems</div></div>
      ...
      <div class="card-grid grid-2">
        <div class="card">
          ...
          ${total > 0 ? `
          <div class="risk-bar">
            ${risk2.critical ? `<div class="segment critical" style="width:${risk2.critical / total * 100}%">${risk2.critical}</div>` : ""}
            ${risk2.high ? `<div class="segment high" style="width:${risk2.high / total * 100}%">${risk2.high}</div>` : ""}
            ${risk2.moderate ? `<div class="segment moderate" style="width:${risk2.moderate / total * 100}%">${risk2.moderate}</div>` : ""}
            ${risk2.low ? `<div class="segment low" style="width:${risk2.low / total * 100}%">${risk2.low}</div>` : ""}
          </div>
          <div class="risk-legend">
            <span><span class="dot" style="background:var(--danger)"></span> Critical (${risk2.critical || 0})</span>
            <span><span class="dot" style="background:#ef4444"></span> High (${risk2.high || 0})</span>
            <span><span class="dot" style="background:var(--warning)"></span> Moderate (${risk2.moderate || 0})</span>
            <span><span class="dot" style="background:var(--success)"></span> Low (${risk2.low || 0})</span>
      ...
      <div class="card-grid grid-2" style="margin-top:16px">
        <div class="card">
          ...
          <table>
            ...
            <tbody>
              ${Object.entries(d.risk_assessments).map(
        ([s, c]) => `<tr><td>${App.badge(s)}</td><td>${c}</td></tr>`
        ...
        <div class="card">
          ...
          ${Object.keys(d.open_incidents).length > 0 ? `<table>
            ...
            <tbody>
              ${Object.entries(d.open_incidents).map(
        ([s, c]) => `<tr><td>${App.badge(s)}</td><td>${c}</td></tr>`
      ).join("")}
            </tbody>
  ...
  // src/frontend/js/pages/assets.js
  var assets = {
    async aiAssets() {
  ...
  // src/frontend/js/pages/risk.js
  var risk = {
    async riskAssessments() {
      let data = { data: [] };
      try {
        data = await API.getRiskAssessments();
      } catch (e) {
      }
      return `
      <div class="page-header">
  ...
  // src/frontend/js/pages/impact.js
  var impact = {
    async impactAssessments() {
      let data = { data: [] };
      try {
        data = await API.getImpactAssessments();
      } catch (e) {
      }
      return `
      <div class="page-header">
  ...
  // src/frontend/js/pages/compliance.js
  var compliance = {
    async compliance() {
  ...
  // src/frontend/js/pages/vendors.js
  var vendors = {
    async vendors() {
      let data = { data: [] };
      try {
        data = await API.getVendorAssessments();
      } catch (e) {
      }
      return `
      <div class="page-header">
  ...
  // src/frontend/js/pages/monitoring.js
  var monitoring = {
    async monitoring() {
      let alerts = { data: [] };
      try {
  ...
  // src/frontend/js/pages/governance.js
  var governance = {
    async maturity() {
      let data = { data: [] };
      try {
        data = await API.getMaturityAssessments();
      } catch (e) {
      }
  ...
  // src/frontend/js/pages/support.js
  var support = {
    async support() {
      let data = { data: [] };
      try {
        data = await API.getSupportTickets();
      } catch (e) {
      }
    ...
    async featureRequests() {
      let data = { data: [] };
      try {
        data = await API.getFeatureRequests({ sort: "votes" });
      } catch (e) {
      }
      const isAdmin = API.user?.role === "admin";
      const categoryLabels = {
  ...
  // src/frontend/js/pages/knowledge.js
  var knowledge = {
    async knowledgeBase() {
      let data = { data: [] };
      try {
        data = await API.getKnowledgeBase();
      } catch (e) {
      }
      const categoryLabels = {
        framework: "Framework",
  ...
  // src/frontend/js/pages/notifications.js
  var notifications = {
    ...
    async readNotification(id) {
      try {
        ...
        items.forEach((el) => {
          if (el.getAttribute("onclick")?.includes(id)) el.classList.remove("unread");
        });
  ...
  // src/frontend/js/pages/training.js
  var training = {
    async training() {
      let modules = { data: [] };
    ...
    async viewTrainingModule(moduleId) {
      ...
      `, m.completed ? "" : '<button class="btn btn-primary" id="tm-complete">Mark as Completed</button>');
        if (!m.completed) {
  ...
  // src/frontend/js/pages/admin.js
  var admin = {
    async users() {
      let data = { data: [] };
      try {
    ...
    `, '<button class="btn btn-primary" id="rp-submit">Reset Password</button>');
      document.getElementById("rp-submit").addEventListener("click", async () => {
        ...
        const confirm2 = document.getElementById("rp-confirm").value;
        if (pw !== confirm2) {
    ...
    async auditLog() {
      let data = { data: [] };
      try {
        data = await API.getAuditLog({ limit: 200 });
      } catch (e) {
      }
      const actionIcons = {
        create: "+",
    ...
    openAuditReport(framework) {
      const url = framework === "all" ? "/api/v1/reports/audit-pack" : `/api/v1/reports/audit-pack?framework=${framework}`;
      const win = window.open("", "_blank");
      fetch(url, { headers: { "Authorization": `Bearer ${API.accessToken}` } }).then((r) => r.text()).then((html) => {
        win.document.write(html);
        win.document.close();
      }).catch(() => {
        win.close();
        App.toast("Failed to generate report", "error");
      });
    },
  ...
  // src/frontend/js/pages/reports.js
  var reports = {
    async reports() {
      ...
      <div class="card" style="margin-bottom:16px">
        ...
        <div id="printable-report">
          <p style="color:var(--text-secondary);margin-bottom:16px">Generated: ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</p>
          <table>
  ...
  // src/frontend/js/pages/events.js
  var events = {
    bindPageEvents(page) {
      switch (page) {
        case "ai-assets":
  ...
  // src/frontend/js/pages/index.js
